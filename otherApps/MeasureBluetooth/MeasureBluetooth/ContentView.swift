...
struct AccumulatingAngle  {
    ...
    func add(_ radians: Double) -> AccumulatingAngle {
        return AccumulatingAngle( m(x,cos(radians)), m(y,sin(radians)), last: radians)
    }
    ...
    var confidenceInt: Int {
        return Int((100*sqrt(x*x + y*y)).rounded())
    }
}
...
class MotionInfo : NSObject, ObservableObject, CLLocationManagerDelegate {
    ...
    func adjust(_ radians: Double) -> Int {
        return Int(180*radians / 3.1415926)
    }
    func stopMotion() {
    ...
    func locationManager(_ manager: CLLocationManager, didUpdateHeading newHeading: CLHeading) {
        ...
        let cHeading = Int(self.orientationAdjustment() + heading)
        self.compassHeading = (cHeading + 720) % 360
    }
    ...
    override init() {
        ...
        if motion.isDeviceMotionAvailable {
            ...
            self.timer = Timer(fire: Date(), interval: (1.0 / 10), repeats: true,
                               block: { (timer) in
